...
export default function Review({data, author}) {
  ...
  function findAuthor(array, name) {
    let found = false;
  ...
  async function removeReview(movieID, author) {
    let worked = false;
    const dataObj = {
      author: author,
      id: movieID,
    ...
    const response = await fetch('https://movie-apixd.herokuapp.com/remove-rating', {
      method: 'POST',
      mode: 'cors',
      cache: 'no-cache',
    ...
    if (response.status === 200) {alert('Successfully removed review'); worked = true}
  ...
  const filteredMovies = data.data
    ...
    .filter(
      movie =>
        (movie.name.toLowerCase().includes(filter) || movie.id.includes(filter)) && findAuthor(movie.ratings, author)
    );
    ...
    <>
      ...
      >
        <div className="container mx-auto px-4 sm:px-8">
          <div className="py-8">
            <div>
              <h2 className="text-2xl font-semibold leading-tight">{author[0].toUpperCase() + author.slice(1)}'s Reviews</h2>
            </div>
                      ...
                      <tr>
                        ...
                        <td className="px-5 py-5 border-b border-gray-200 bg-white text-sm">
                          <p
                            ...
                            style={{ textAlign: "center" }}
                          >
                            {movie.ratings.find(rating => rating.author === author).rating}
                          </p>
                        </td>
                        <td
                          ...
                          style={{ textAlign: "center" }}
                        >
                          {movie.ratings.find(rating => rating.author === author).comment}
                        </td>
                        <td className="px-5 py-5 border-b border-gray-200 bg-white text-sm">
                          <svg
                            style={{
                            ...
                            onClick={async () =>
                              (await removeReview(movie.id, author))
